...
def run(
    ...
    # Define o intervalo de tempo (24h format)
    inicio = dt_time(1, 59)    # 2:00
    fim = dt_time(7, 1)      # 7:30
    #inicio = dt_time(13, 59)    # 2:00
    #fim = dt_time(23, 59)      # 7:30
    ...
    while True:
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            if (inicio <= now) and (now <= fim):      
                    ...
                    if len(det):
                        ...
                        for c in det[:, 5].unique():
                            ...
                            if c == 0: #person
                                ...
                                current_datetime_file = datetime.now().strftime("%Y_%m_%d_%H_%M_%S_%f")
                                current_datetime_text = datetime.now().strftime("%Y/%m/%d-%H:%M:%S:%f")
                                ...
                                mixer.init()
                                mixer.music.load('./sounds/dog.mp3')
                                mixer.music.play()
                                time.sleep(10)
                ...
                cv2.resizeWindow(str(p), int(im0.shape[1]/2), int(im0.shape[0]/2))
                cv2.imshow(str(p), image_detection)
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'test_video', help='file/dir/URL/glob/screen/0(webcam)')
