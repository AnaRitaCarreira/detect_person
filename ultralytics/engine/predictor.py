...
class BasePredictor:
    ...
    def write_results(self, i: int, p: Path, im: torch.Tensor, s: List[str]) -> str:
        ...
        # print aqui
        if len(result.boxes) > 0:  # Exemplo: print se houver pelo menos uma detecção
            print(f"Detecção encontrada em {p.name}: {len(result.boxes)} objetos detectados.")

            # result.boxes.cls contém os índices das classes detectadas
            classes_detectadas = [result.names[int(cls)] for cls in result.boxes.cls]

            print(f"Detecções em {p.name}: {classes_detectadas}")
